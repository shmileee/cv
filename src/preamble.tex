...
\ifxetexorluatex
  ...
  % above, out of the pinned bundle, so a laptop and a CI runner resolve the
  % identical font. Plex Mono is also what oponomarov.com sets its own code
  % column in, which is the point -- the CV and the site it points at should
  % render that domain the same way.
  %
...
% Monochrome palette. The class expects these four color aliases; the names
...
% The class trails every contact entry with 2em. With six entries that wrapped
% into a tidy 3+3; with four it left GitHub orphaned on a line of its own.
% 1.5em is the widest gap that still fits all four on one line.
...
% predates upstream's `withhyper` option, so they are wrapped here. \email and
% \phone stay defined but unused, ready if a contact address is added back.
